...
partial model Pipe_ParamVariants
  "New version of pipe base class to select parametrisation from drop down menu"
    //Parameter Model
    extends ModelicaAssessment.Templates.PipeGeometryParameters;

    // Interfaces
    .ModelicaAssessment2.Interfaces.Inlet inlet annotation (Placement(
        transformation(
        extent={{-110.0,-10.0},{-90.0,10.0}},
        origin={0.0,0.0},
        rotation=0.0)));
    .ModelicaAssessment2.Interfaces.Outlet outlet annotation (Placement(
        transformation(
        extent={{90.0,-10.0},{110.0,10.0}},
        origin={0.0,0.0},
        rotation=0.0)));
   ...
   // Modelica.Units.SI.SpecificEnthalpy h= inlet.h_outflow "Specific Enthalpy leaving the component";
